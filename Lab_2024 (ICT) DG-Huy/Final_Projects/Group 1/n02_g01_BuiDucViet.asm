...
.data
circle_end:	.word	1	# Pointer point to the end of the cycle
circle:		.word		# Pointer point to the array storing data of the cycle
.text
initial_declare:
	addi	$s0, $0, 255	# x = 255
	addi	$s1, $0, 255	# y = 255
...
right:	
	bne	$s2, 1, left #If the ball move to left, don't check with the right edge, check with the left edge
	j	check_right
...
move_circle:
	...
	add	$s0, $s0, $s2 	# Update new value for center point of the cycle
	add	$s1, $s1, $s3 
...
check_right:
	add	$t0, $s0, $s4	# Set $t0 to the right side of the circle (center + R)
